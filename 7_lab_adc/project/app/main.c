...
#define BIT_2_MASK 0x03
#define BIT_1_MASK 0x01
...
int main(void)
{
    /// STUDENTS: To be programmed
		...
		uint16_t hex_switch_value;
		uint16_t value;
		adc_init();
		while(1){
			
			hex_switch_value = CT_HEXSW & BIT_2_MASK;
			
			
			switch(hex_switch_value){
				case 0x0:
					resolution = ADC_RES_6BIT;
					CT_LED->BYTE.LED7_0 = 0x3F;
			...
			if(CT_DIPSW->BYTE.S31_24 & (BIT_1_MASK << 7)){
