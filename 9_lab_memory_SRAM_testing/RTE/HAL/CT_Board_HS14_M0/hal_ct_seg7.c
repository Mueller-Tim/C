...
#define LCD_DISP_CLEAR_MASK                 0xffu
#define LCD_ALL_CLEAR_MASK                  0xffffffffu
#define CHECK_ARRAY_OUT_OF_BOUNDS_BCD       0x64u
#define CHECK_ARRAY_OUT_OF_BOUNDS_SEG7      0x10u
...
void hal_ct_seg7_raw_write(hal_ct_seg7_t display,
                           uint8_t segment)
{
    assert(display == HAL_CT_SEG7_DS0 || display == HAL_CT_SEG7_DS1 || 
            display == HAL_CT_SEG7_DS2 || display == HAL_CT_SEG7_DS3);
    switch (display) {
...
{
    CT_SEG7->RAW.WORD &= ~((uint32_t) HAL_CT_SEG7_SEGDP << display);
}
...
uint8_t hal_ct_seg7_get_bcd(uint8_t value)
{
    if (value >= CHECK_ARRAY_OUT_OF_BOUNDS_BCD){
        ...
        return (uint8_t) ((value % 10u) | ((value / 10u) << 4u));
    }
}
